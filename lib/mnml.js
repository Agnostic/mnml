...
(function(root){
  ...
  function loadTemplate(path, elm){
    ...
    xhr.onload = function(){
      ...
      template = template.replace( new RegExp( "\{\{ (.*) \}\}", "gi" ), '<span model="$1"></span>' );
      template = template.replace( new RegExp( "\{\{(.*)\}\}", "gi" ), '<span model="$1"></span>' );
  ...
  M.prototype.checkRoutes = function(){
    ...
    for(var i = 0; i < self.routes.length; i++){
      ...
      if(self.currentRoute.path !== route.path){
        ...
        if(regex.test(History.getState().data.url) && route.template){
          var params  = regex.exec(History.getState().data.url);
          temp_params = [];

          params.forEach(function(item){
            if( temp_params.indexOf(item) < 0 ){
